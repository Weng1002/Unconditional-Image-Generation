...
# 載入已訓練過的 model
checkpoint_path = f"{save_dir}/model_epoch_500.pt"
if os.path.exists(checkpoint_path):
    model.load_state_dict(torch.load(checkpoint_path, map_location=device))
    print(f"成功載入模型參數：{checkpoint_path}")
else:
    print(f"找不到 checkpoint：{checkpoint_path}，將從頭開始訓練。")
...
# for epoch in range(num_epochs):
    ...
    if (epoch + 1) % 50 == 0:
        ...
        torch.save(ema_model.state_dict(), f"{save_dir}/final_model_{epoch+1}_EMA.pt")
        with torch.no_grad():
            ...
            save_image(sampled, f"{img_test}/sample_epoch_{epoch+1:03d}.png", nrow=4)
       
            
torch.save(model.state_dict(), f"{save_dir}/final_model.pt")
torch.save(ema_model.state_dict(), f"{save_dir}/final_model_EMA.pt")  
print("已儲存最終模型 final_model.pt")
